...
// ===============================
function updateStatusTampilan(teksStatus, iconEmoji = "🟢", pesanAktivitas = "") {
    // 1. Update Teks Status Hari Ini (di card bagian atas)
    ...
    // 3. Update Teks "Belum Ada Aktivitas" di Card Bawah
    const elHasil = document.getElementById("hasil");
    if (elHasil) {
        if (pesanAktivitas) {
            elHasil.textContent = pesanAktivitas;
        } else {
            // Jika tidak ada pesan baru, coba ambil dari elemen atau isi default
            if (elHasil.textContent.trim() === "" || elHasil.textContent.includes("Belum Ada Aktivitas")) {
                elHasil.textContent = teksStatus;
            }
        }
    }

    // 4. Simpan ke localStorage agar tidak hilang saat di-refresh
    const hariIniStr = new Date().toISOString().split('T')[0];
    localStorage.setItem("statusAbsen_" + hariIniStr, JSON.stringify({
        teks: teksStatus,
        icon: iconEmoji,
        aktivitas: pesanAktivitas || teksStatus
    }));
}

// Muat status tersimpan dari localStorage saat halaman dimuat ulang (Refresh)
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (savedData) {
        try {
            ...
            updateStatusTampilan(parsed.teks, parsed.icon, parsed.aktivitas);
        } catch (e) {
...
if (btnMasuk) {
  btnMasuk.addEventListener("click", function () {
    ...
    .then(data => {
      ...
      const pesanAktivitas = `Absen Masuk Berhasil pada pukul ${jamMasuk} WIB`;

      // Update tampilan status atas dan teks aktivitas bawah
      updateStatusTampilan(statusText, "🟢", pesanAktivitas);
    ...
    .catch(err => {
      ...
      // Tetap update tampilan lokal meskipun respon Google Apps Script terlambat/error CORS
      const statusText = `Sudah Absen Masuk (${jamMasuk || "Masuk"})`;
      const pesanAktivitas = `Absen Masuk Dikirim pada pukul ${jamMasuk} WIB`;
      updateStatusTampilan(statusText, "🟢", pesanAktivitas);

      alert("Absen Masuk Berhasil Dikirim!");
    });
  });
}
...
if (btnPulang) {
  btnPulang.addEventListener("click", function () {
    ...
    .then(data => {
      ...
      const pesanAktivitas = `Absen Pulang Berhasil pada pukul ${jamPulang} WIB`;
    ...
    .catch(err => {
      ...
      const statusText = `Sudah Absen Pulang (${jamPulang})`;
      const pesanAktivitas = `Absen Pulang Dikirim pada pukul ${jamPulang} WIB`;
      updateStatusTampilan(statusText, "🔵", pesanAktivitas);

      alert("Absen Pulang Berhasil Dikirim!");
    });
  });
}